...
const Navbar = () => {
  ...
  return (
    <div className='w-screen h-[80px] bg-[#1B1A17] px-[30px] sm:px-[50px] md:px-[80px] lg:px-[100px] xl:px-[150px] 2xl:px-[300px] flex items-center justify-between box-border'>
      <Link to={'/JAP/'}><img className='w-[80px] sm:w-[100px] md:w-[120px] lg:w-[140px] xl:w-[180px] 2xl:w-[210px] h-auto' src={JapLogo} alt="<JAP/>" /></Link>
      ...
      <div className={`h-screen w-3/4 bg-white absolute top-0 right-0 drop-shadow-md flex flex-col justify-start items-end ${navOpen ? 'block' : 'hidden'} lg:flex lg:relative lg:h-auto lg:w-auto lg:flex-row lg:bg-transparent lg:text-white`}>
        <button onClick={() => setNavOpen(false)} className='pr-[35px] pt-[28px] hover:text-[#6B6A66] text-[#1B1A17] font-bold font-sans text-center text-[20px] pb-1 lg:hidden'>X</button>
        <div className='w-full font-sans pt-10 lg:flex lg:items-center lg:h-full lg:justify-center lg:pt-0'>
          ...
          <Link to='/JAP/store'><p className='flex items-center w-full h-[40px] text-[#1B1A17] lg:text-white hover:bg-[#2E2D28] lg:hover:bg-transparent hover:text-[#F0A500] hover:border-l-4 lg:hover:border-l-0 lg:hover:border-b-4 hover:border-[#F0A500]  px-[20px]'>STORE</p></Link>
        </div>
